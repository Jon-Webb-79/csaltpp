/* simd_sse3_char.inl
   SSE3 path uses SSE2 integer ops for this routine.
   Requires: <pmmintrin.h> and <emmintrin.h>
*/
#ifndef CSALT_SIMD_SSE3_CHAR_INL
#define CSALT_SIMD_SSE3_CHAR_INL
                                        ...
                                        size_t n)
{
    ...
    while ((i + 16u) <= n) {
        ...
        if (m != (uint16_t)0xFFFFu) {
            const uint16_t mism = (uint16_t)((uint16_t)0xFFFFu ^ m); /* 1 where different */
...
static inline size_t simd_last_index_u8_sse3(const unsigned char* s, size_t n, unsigned char c) {
...
static inline size_t simd_first_substr_index_sse3(const unsigned char* s, size_t n,
...
static inline int csalt_hi_bit32_sse3(uint32_t m) { return 31 - __builtin_clz(m); }

static inline size_t simd_last_substr_index_sse3(const unsigned char* s, size_t n,
                                                 const unsigned char* pat, size_t m) {
    if (m == 0) return n;
    ...
    while (i + 16 <= n) {
        ...
        while (mask) {
            int pos = csalt_hi_bit32_sse3(mask);
            size_t cand = i + (size_t)pos;
