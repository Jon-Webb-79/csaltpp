...
/**
 * @class StringKey
 * @brief Fixed-size trivially copyable string key for use with Dict<K, V>
 *
 * @details `cslt::String` cannot be used directly as a Dict key because its
 *          private destructor and deleted copy operations make it
 *          non-trivially-copyable — a requirement enforced by Dict's
 *          `static_assert`.  `StringKey<N>` solves this by copying the string
 *          content into a fixed-size stack buffer of `N` bytes (including the
 *          null terminator), making the type trivially copyable and zero-cost
 *          to hash.
 *
 *          The template parameter `N` is the total buffer size **including** the
 *          null terminator, so a `StringKey<32>` stores up to 31 printable
 *          characters.  Strings longer than `N-1` characters are silently
 *          truncated.
 *
 *          `operator==` uses `std::strncmp` so Dict's equality detection trait
 *          picks it up automatically — no custom comparator is needed.
 *
 * Recommended sizes:
 ...
 *
 * @tparam N Total buffer size in bytes, including the null terminator.
 *           Must be >= 2.
 *
 * @code{.cpp}
 * // Type alias to keep call sites concise
...
struct StringKey {
    ...
    static_assert(N >= 2u,
        "StringKey<N>: N must be >= 2 to hold at least one character "
        "plus the null terminator.");
 
    char data[N];   ///< Null-terminated fixed-size character buffer
 
    /**
     * @brief Construct from a null-terminated C-string
     *
     * @param s  Source string.  If nullptr, the key is initialised to an
     *           empty string.  If longer than N-1 characters the content is
     *           silently truncated.
     */
    explicit StringKey(const char* s) noexcept {
        std::memset(data, 0, N);
        if (s) std::strncpy(data, s, N - 1u);
    }
 
    /**
     * @brief Construct from a cslt::String
     *
     * @param s  Source String.  If its content is longer than N-1 characters
     *           it is silently truncated.
     *
     * @note This constructor is only available when string.hpp has been
     *       included before dict.hpp.  If you do not use cslt::String as a
     *       source, include only dict.hpp and construct from a C-string
     *       literal instead.
     */
    template <typename StringT>
    explicit StringKey(const StringT& s) noexcept {
        std::memset(data, 0, N);
        if (s.c_str()) std::strncpy(data, s.c_str(), N - 1u);
    }
 
    /**
     * @brief Equality comparison — required by Dict's key equality detection
     *
     * @return true if both keys hold the same null-terminated string content
     */
    bool operator==(const StringKey<N>& other) const noexcept {
        return std::strncmp(data, other.data, N) == 0;
    }
 
    /**
     * @brief Return the stored string as a C-string pointer
     */
    const char* c_str() const noexcept { return data; }
 
    /**
     * @brief Return the logical length of the stored string (excludes null terminator)
     */
