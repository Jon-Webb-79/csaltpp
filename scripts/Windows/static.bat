...
REM - Static lib via BUILD_SHARED_LIBS=OFF (csaltpp target -> csaltpp.lib)
REM
REM Usage:
REM   static.bat
REM   static.bat --prefix "C:\SDK\csaltpp"
...
REM --------------------- Configure --------------------
REM With BUILD_SHARED_LIBS=OFF, 'csaltpp' is a static library target (when sources exist).
REM If header-only, this still installs headers/.inl and CMake package files.
set CMAKE_ARGS=-S "%SRC_DIR%" -B "%OUT_DIR%" -DCMAKE_BUILD_TYPE=%BUILD_TYPE% -DCMAKE_INSTALL_PREFIX="%PREFIX%" -DBUILD_SHARED_LIBS=OFF -DCSALTXX_BUILD_TESTS=OFF -DCSALTXX_BUILD_STATIC=OFF
...
echo     Library: %PREFIX%\lib\csaltpp.lib (only if compiled sources exist; generator-dependent)
exit /b 0
